...
.. important::
    ...
    Don't call your script ``herokutl.py``! Python will try to import
...
At last, we create a new `TelegramClient <herokutl.client.telegramclient.TelegramClient>`
...
.. note::
    ...
    Since herokutl is an asynchronous library, you need to ``await``
...
automatically `start() <herokutl.client.auth.AuthMethods.start>` the client,
...
You can also use herokutl for your bots (normal bot accounts, not users).
...
.. code-block:: python
    ...
    from herokutl import TelegramClient, connection
