...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=188&to=193>`__.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=181&to=188>`__.

Additions
~~~~~~~~~

* Support for CDN downloads should be back. herokutl still prefers no CDN by default.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=178&to=181>`__.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=173&to=178>`__.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=167&to=173>`__.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=166&to=167>`__.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=165&to=166>`__.
...
.. code-block:: html
  ...
  <pre>
    <code class='language-python'>from herokutl import TelegramClient</code>
  </pre>

Note that herokutl v1's markdown is a custom format and won't support language tags.
...
This will allow herokutl to detect the correct extension (and file type).
...
* ``imghdr`` is deprecated in newer Python versions, so herokutl no longer uses it.
  This means there might be some cases where herokutl fails to infer the file extension for buffers containing images.
...
* ``progress_callback`` is now called when dealing with albums. See the
  documentation on `client.send_file() <herokutl.client.uploads.UploadMethods.send_file>`
  for details.
...
* ``force_sms`` and ``sign_up`` have been deprecated. See `issue 4050`_ for details.
  ...
  herokutl, to use those features.
...
.. _issue 4050: https://github.com/LonamiWebs/herokutl/issues/4050
...
* ``herokutl.types`` could not be used as a module.
...
https://github.com/LonamiWebs/herokutl/compare/v1.23.0...v1.24.0
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=129&to=130>`__.

Enhancements
~~~~~~~~~~~~

* `client.pin_message() <herokutl.client.messages.MessageMethods.pin_message>`
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=125&to=129>`__.

Enhancements
~~~~~~~~~~~~

* You can now specify a message in `client.get_stats()
  <herokutl.client.chats.ChatMethods.get_stats>`.
...
* Get default chat permissions by not supplying a user to `client.get_permissions()
  <herokutl.client.chats.ChatMethods.get_permissions>`.
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=124&to=125>`__.
...
`should fix it <https://github.com/LonamiWebs/herokutl/issues/1724>`__.

Additions
~~~~~~~~~

* `Message.click() <herokutl.tl.custom.message.Message.click>` now supports
...
* `Message.edit <herokutl.tl.custom.message.Message.edit>` wasn't working in
  your own chat on events other than ``NewMessage``.
* `client.delete_dialog() <herokutl.client.dialogs.DialogMethods.delete_dialog>`
...
`View new and changed raw API methods <https://diff.herokutl.dev/?from=122&to=124>`__.

A bit late to the party, but herokutl now offers a convenient way to comment
...
to arrive because nobody noticed that herokutl was using a
...
* New ``comment_to`` parameter in `client.send_message()
  <herokutl.client.messages.MessageMethods.send_message>`, and
  `client.send_file() <herokutl.client.uploads.UploadMethods.send_file>`
...
herokutl's focus is being an asyncio-based, pure-Python implementation to
interact with Telegram's API. Streaming audio is beyond the current scope of
the project and would be a big undertaking.

However, that doesn't mean calls are not possible with herokutl. If you want
to help design a Python library to perform audio calls, which can then be used
with herokutl (so you can use herokutl + that new library to perform calls
with herokutl), please refer to `@pytgcallschat <https://t.me/pytgcallschat/>`__
...
<https://t.me/herokutlChat/284717>`__, if you wish to discuss it further.
...
* `client.kick_participant() <herokutl.client.chats.ChatMethods.kick_participant>`
...
* `events.ChatAction <herokutl.events.chataction.ChatAction>` should misbehave
  less.
...
transparently by herokutl so you don't need to worry about it.

Additions
~~~~~~~~~

* New `client.set_proxy()
  <herokutl.client.telegrambaseclient.TelegramBaseClient.set_proxy>` method
...
* New method to unpin messages `client.unpin_message()
  <herokutl.client.messages.MessageMethods.unpin_message>`.
...
* If the ``python-socks`` library is installed (new optional requirement), it
  ...
  `asyncio`.
* `client.send_file() <herokutl.client.uploads.UploadMethods.send_file>` will
...
use herokutl's friendly methods and custom properties, which have a higher
...
`Message.reply_to_msg_id <herokutl.tl.custom.message.Message.reply_to_msg_id>`
and `Message.to_id <herokutl.tl.custom.message.Message.to_id>` that behave
...
* You can no longer assign to `Message.reply_to_msg_id
  <herokutl.tl.custom.message.Message.reply_to_msg_id>` and `Message.to_id
  <herokutl.tl.custom.message.Message.to_id>` because these are now properties
...
* New `client.get_permissions() <herokutl.client.chats.ChatMethods.get_permissions>`
...
* `Message.click() <herokutl.tl.custom.message.Message.click>` now works on
  normal polls!
* New ``local_addr`` parameter in the client constructor to use a specific
  local network address when connecting to Telegram.
* `client.inline_query() <herokutl.client.bots.BotMethods.inline_query>` now
...
* You can now get comments in a channel post with the ``reply_to`` parameter in
  `client.iter_messages() <herokutl.client.messages.MessageMethods.iter_messages>`.
...
* In some cases, there were issues when using `events.Album
  <herokutl.events.album.Album>` together with `events.Raw
  <herokutl.events.raw.Raw>`.
* For some channels, one of their channel photos would not show up in
  `client.iter_profile_photos() <herokutl.client.chats.ChatMethods.iter_profile_photos>`.
...
* Manually using `client.sign_up <herokutl.client.auth.AuthMethods.sign_up>`
...
* When trying to `client.start() <herokutl.client.auth.AuthMethods.start>` to
...
<herokutl.client.chats.ChatMethods.get_stats>`. This way you'll be able
...
* `client.upload_file() <herokutl.client.uploads.UploadMethods.upload_file>`
...
* New `client.qr_login() <herokutl.client.auth.AuthMethods.qr_login>`.
* `message.click <herokutl.tl.custom.message.Message.click>` now lets you
...
* Updated documentation and list of known errors.
* `events.Album <herokutl.events.album.Album>` should now handle albums from
  different data centers more gracefully.
* `client.download_file()
  <herokutl.client.downloads.DownloadMethods.download_file>` now supports
...
* New `Message.dice <herokutl.tl.custom.message.Message.dice>` property.
* The ``func=`` parameter of events can now be an ``async`` function.

Bug fixes
~~~~~~~~~

* Fixed `client.action() <herokutl.client.chats.ChatMethods.action>`
...
* Fixed send methods on `client.conversation()
  <herokutl.client.dialogs.DialogMethods.conversation>` were not honoring
  cancellation.
...
* The service message produced by `Message.pin()
  <herokutl.tl.custom.message.Message.pin>` is now returned.
* Sending a file with `client.send_file()
  <herokutl.client.uploads.UploadMethods.send_file>` now works fine when
  you pass an existing dice media (e.g. sending a message copy).
* `client.edit_permissions() <herokutl.client.chats.ChatMethods.edit_permissions>`
...
* Passing ``None`` as the entity to `client.delete_messages()
  <herokutl.client.messages.MessageMethods.delete_messages>` would fail.
...
* Getting input users would sometimes fail on `events.ChatAction
  <herokutl.events.chataction.ChatAction>`.
...
* Fix `client.get_entity() <herokutl.client.users.UserMethods.get_entity>`
...
* Fixed some issues with `events.Album <herokutl.events.album.Album>`.
* Fixed some issues with `client.kick_participant()
  <herokutl.client.chats.ChatMethods.kick_participant>` and
  `client.edit_admin() <herokutl.client.chats.ChatMethods.edit_admin>`.
* Fixed sending albums and more within `client.conversation()
  <herokutl.client.dialogs.DialogMethods.conversation>`.
...
.. code-block:: python
    ...
    # Remind yourself to walk the dog in 10 minutes (after you play with herokutl's update)
    await client.send_message('me', 'Walk the dog',
                              schedule=timedelta(minutes=10))

    # Remind your friend tomorrow to update herokutl
    await client.send_message(friend, 'Update herokutl!',
                              schedule=timedelta(days=1))

Additions
~~~~~~~~~

* New `Button.auth <herokutl.tl.custom.button.Button.auth>` friendly button
  you can use to ask users to login to your bot.
* herokutl's repository now contains ``*.nix`` expressions that you can use.
* New `client.kick_participant() <herokutl.client.chats.ChatMethods.kick_participant>`
  method to truly kick (not ban) participants.
* New ``schedule`` parameter in `client.send_message()
  <herokutl.client.messages.MessageMethods.send_message>`, `client.edit_message()
  <herokutl.client.messages.MessageMethods.edit_message>`, `client.forward_messages()
  <herokutl.client.messages.MessageMethods.forward_messages>` and `client.send_file()
  <herokutl.client.uploads.UploadMethods.send_file>`.

Bug fixes
~~~~~~~~~

* Fix calling ``flush`` on file objects which lack this attribute.
* Fix `CallbackQuery <herokutl.events.callbackquery.CallbackQuery>` pattern.
* Fix `client.action() <herokutl.client.chats.ChatMethods.action>` not returning
  itself when used in a context manager (so the ``as`` would be `None`).
* Fix sending :tl:`InputKeyboardButtonUrlAuth` as inline buttons.
* Fix `client.edit_permissions() <herokutl.client.chats.ChatMethods.edit_permissions>`
  defaults.
* Fix `Forward <herokutl.tl.custom.forward.Forward>` had its ``client`` as `None`.
...
* Fix `client.get_messages() <herokutl.client.messages.MessageMethods.get_messages>`
  when being passed more than 100 IDs.
* Fix `Message.document <herokutl.tl.custom.message.Message.document>`
...
* Documentation now avoids ``herokutl.sync``, which should hopefully be less confusing.
* Better error messages for flood wait.
* You can now `client.get_drafts() <herokutl.client.dialogs.DialogMethods.get_drafts>`
  for a single entity (which means you can now get a single draft from a single chat).
* New-style file IDs now work with herokutl.
* The ``progress_callback`` for `client.upload_file()
  <herokutl.client.uploads.UploadMethods.upload_file>` can now be an ``async def``.
...
With the layer 103, herokutl is now able to send and receive animated
...
* New `events.Album <herokutl.events.album.Album>` to easily receive entire albums!
* New `client.edit_admin() <herokutl.client.chats.ChatMethods.edit_admin>`
  and `client.edit_permissions() <herokutl.client.chats.ChatMethods.edit_permissions>`
  methods to more easily manage your groups.
* New ``pattern=`` in `CallbackQuery
  <herokutl.events.callbackquery.CallbackQuery>`.
* New `conversation.cancel_all()
  <herokutl.tl.custom.conversation.Conversation.cancel>` method,
  to cancel all currently-active conversations in a particular chat.
* New `herokutl.utils.encode_waveform` and `herokutl.utils.decode_waveform`
...
* New ``ignore_pinned`` parameter in `client.iter_dialogs()
  <herokutl.client.dialogs.DialogMethods.iter_dialogs>`.
* New `Message.mark_read() <herokutl.tl.custom.message.Message.mark_read>`
  method.
...
* Fixed `client.iter_dialogs()
  <herokutl.client.dialogs.DialogMethods.iter_dialogs>` missing some under
...
* Fixed invalid states in `Conversation
  <herokutl.tl.custom.conversation.Conversation>`.
* Fixes to some methods in utils regarding extensions.
* Fixed memory cycle in `Forward <herokutl.tl.custom.forward.Forward>`
...
* Fixed `File.ext <herokutl.tl.custom.file.File.ext>` not working on
  unknown mime-types, despite the file name having the extension.
* Fixed ``ids=..., reverse=True`` in `client.iter_messages()
  <herokutl.client.messages.MessageMethods.iter_messages>`.
* Fixed `Draft <herokutl.tl.custom.draft.Draft>` not being aware
  of the entity.
* Added missing re-exports in ``herokutl.sync``.

Enhancements
~~~~~~~~~~~~

* Improved `conversation.cancel()
  <herokutl.tl.custom.conversation.Conversation.cancel>`
  behaviour. Now you can use it from anywhere.
* The ``progress_callback`` in `client.download_media()
  <herokutl.client.downloads.DownloadMethods.download_media>`
  now lets you use ``async def``.
* Improved documentation and the online
  method reference at https://tl.herokutl.dev.
...
* The way photos are downloaded changed during the layer update of the
  ...
  `client.download_media() <herokutl.client.downloads.DownloadMethods.download_media>`
...
* New `Message.file <herokutl.tl.custom.message.Message.file>` property!
  Now you can trivially access `message.file.id  <herokutl.tl.custom.file.File.id>`
  to get the file ID of some media, or even ``print(message.file.name)``.
* Archiving dialogs with `Dialog.archive() <herokutl.tl.custom.dialog.Dialog.archive>`
  or `client.edit_folder() <herokutl.client.dialogs.DialogMethods.edit_folder>`
  is now possible.
* New cleaned-up method to stream downloads with `client.iter_download()
  <herokutl.client.downloads.DownloadMethods.iter_download>`, which offers
  a lot of flexibility, such as arbitrary offsets for efficient seeking.
* `Dialog.delete() <herokutl.tl.custom.dialog.Dialog.delete>` has existed
  for a while, and now `client.delete_dialog()
  <herokutl.client.dialogs.DialogMethods.delete_dialog>` exists too so you
...
* Some people or chats have a lot of profile photos. You can now iterate
  ...
  <herokutl.client.chats.ChatMethods.iter_profile_photos>` method.
* You can now annoy everyone with the new `Message.pin(notify=True)
  <herokutl.tl.custom.message.Message.pin>`! The client has its own
  variant too, called `client.pin_message()
  <herokutl.client.messages.MessageMethods.pin_message>`.
...
* `conv.get_response() <herokutl.tl.custom.conversation.Conversation.get_response>`
  after ID 0 wasn't allowed when it should.
* `InlineBuilder <herokutl.tl.custom.inlinebuilder.InlineBuilder>` only worked
...
* `client.iter_participants(search=...) <herokutl.client.chats.ChatMethods.iter_participants>`
...
* ``herokutl`` now re-export all the goodies that you commonly need when
  using the library, so e.g. ``from herokutl import Button`` will now work.
* ``herokutl.sync`` now re-exports everything from ``herokutl``, so that
...
If you have been using herokutl for a while, you probably know how annoying
...
* `client.disconnect()
  <herokutl.client.telegrambaseclient.TelegramBaseClient.disconnect>`
...
* New in-memory cache for input entities. This should mean a lot less
  of disk look-ups.
* New `client.action <herokutl.client.chats.ChatMethods.action>` method
...
* Fix `client.edit_message()
  <herokutl.client.messages.MessageMethods.edit_message>`
...
* Attempt at fixing `client.iter_dialogs()
  <herokutl.client.dialogs.DialogMethods.iter_dialogs>` missing many dialogs.
* ``offset_date`` in `client.iter_messages()
  <herokutl.client.messages.MessageMethods.iter_messages>` was being
  ignored in some cases. This has been worked around.
* Fix `callback_query.edit()
  <herokutl.events.callbackquery.CallbackQuery.Event.edit>`.
* Fix `CallbackQuery(func=...) <herokutl.events.callbackquery.CallbackQuery>`
  was being ignored.
* Fix `UserUpdate <herokutl.events.userupdate.UserUpdate>` not working for
...
* Fix the ``blacklist_chats`` parameter in `MessageRead
  <herokutl.events.messageread.MessageRead>` not working as intended.
...
* Fix sending albums with `client.send_file()
  <herokutl.client.uploads.UploadMethods.send_file>` was not returning
  the sent messages.
* Fix forwarding albums with `client.forward_messages()
  <herokutl.client.messages.MessageMethods.forward_messages>`.
...
* You can now forward messages from different chats directly with
  `client.forward_messages <herokutl.client.messages.MessageMethods.forward_messages>`.
...
* You can now pass ``phone`` and ``phone_code_hash`` in `client.sign_up
  <herokutl.client.auth.AuthMethods.sign_up>`, although you probably don't
  need that.
...
* Fix `herokutl.utils.resolve_bot_file_id`, which wasn't working after
...
* Fix `takeout() <herokutl.client.account.AccountMethods.takeout>` failing
  when they need to download media from other DCs.
* Fix repeatedly calling `conversation.get_response()
  <herokutl.tl.custom.conversation.Conversation.get_response>` when many
  messages arrived at once (i.e. when several of them were forwarded).
* Fixed connecting with `ConnectionTcpObfuscated
  <herokutl.network.connection.tcpobfuscated.ConnectionTcpObfuscated>`.
* Fix `client.get_peer_id('me')
  <herokutl.client.users.UserMethods.get_peer_id>`.
...
* Fix a strange error when using too many IDs in `client.delete_messages()
  <herokutl.client.messages.MessageMethods.delete_messages>`.
* Fix `client.send_file <herokutl.client.uploads.UploadMethods.send_file>`
  with the result of `client.upload_file
  <herokutl.client.uploads.UploadMethods.upload_file>`.
* When answering inline results, their order was not being preserved.
* Fix `events.ChatAction <herokutl.events.chataction.ChatAction>`
...
* Added new hot-keys to the online documentation at
  https://tl.herokutl.dev/ such as ``/`` to search.
...
* `message.edit <herokutl.tl.custom.message.Message.edit>` now respects
...
* You can now "hide_via" when clicking on results from `client.inline_query
  <herokutl.client.bots.BotMethods.inline_query>` to @bing and @gif.
* You can now further configure the logger herokutl uses to suit your needs.
...
* Reworked README and examples at
  https://github.com/LonamiWebs/herokutl/tree/master/herokutl_examples
...
through `client.takeout() <herokutl.client.account.AccountMethods.takeout>`.
...
* The new `AdminLogEvent <herokutl.tl.custom.adminlogevent.AdminLogEvent>`
  had a bug that made it unusable.
* `client.iter_dialogs() <herokutl.client.dialogs.DialogMethods.iter_dialogs>`
...
* New `client.iter_admin_log()
  <herokutl.client.chats.ChatMethods.iter_admin_log>` method.

Bug fixes
~~~~~~~~~

* `client.is_connected()
  <herokutl.client.telegrambaseclient.TelegramBaseClient.is_connected>`
...
* Fixed `client.get_input_entity()
  <herokutl.client.users.UserMethods.get_input_entity>` for integer IDs
...
* You can now configure the reply markup when using `Button
  <herokutl.tl.custom.button.Button>` as a bot.
* More properties for `Message
  <herokutl.tl.custom.message.Message>` to make accessing media convenient.
...
This layer brings polls, and you can create and manage them through herokutl!
...
* New `client.is_bot() <herokutl.client.users.UserMethods.is_bot>` method.
...
* `button.click() <herokutl.tl.custom.messagebutton.MessageButton.click>`
...
* Fixed `message.get_reply_message
  <herokutl.tl.custom.message.Message.get_reply_message>`
  as a bot when a user replied to a different bot.
* Accessing some document properties in a `Message
  <herokutl.tl.custom.message.Message>` would fail.

Enhancements
~~~~~~~~~~~~

* Accessing `events.ChatAction <herokutl.events.chataction.ChatAction>`
...
* Fixed ``herokutl.sync`` when using other threads.
...
* Better default behaviour for `client.send_read_acknowledge
  <herokutl.client.messages.MessageMethods.send_read_acknowledge>`.
...
* The ``callback`` parameter from `herokutl.tl.custom.button.Button.inline()`
...
* New `dialog.delete() <herokutl.tl.custom.dialog.Dialog.delete>` method.
...
* Fixed `Conversation.wait_event()
  <herokutl.tl.custom.conversation.Conversation.wait_event>`.
* Fixed replying with photos/documents on inline results.
* `client.is_user_authorized()
  <herokutl.client.users.UserMethods.is_user_authorized>` now works
  correctly after `client.log_out()
  <herokutl.client.auth.AuthMethods.log_out>`.
* `dialog.is_group <herokutl.tl.custom.dialog.Dialog>` now works for
  :tl:`ChatForbidden`.
* Not using ``async with`` when needed is now a proper error.
* `events.CallbackQuery <herokutl.events.callbackquery.CallbackQuery>`
  with string regex was not working properly.
* `client.get_entity('me') <herokutl.client.users.UserMethods.get_entity>`
  now works again.
...
* Support ``next_offset`` in `inline_query.answer()
  <herokutl.events.inlinequery.InlineQuery.Event.answer>`.
...
This will `register <herokutl.events.register>` the ``handler`` callback
...
* Fix strange behavior for `send_message(chat, Message, reply_to=foo)
  <herokutl.client.messages.MessageMethods.send_message>`.
...
* `iter_participants <herokutl.client.chats.ChatMethods.iter_participants>`
...
* Callbacks for `herokutl.client.auth.AuthMethods.start` can be ``async``.
...
such as the new `Conversation <herokutl.tl.custom.conversation.Conversation>`.
...
- Implemented global search through `client.iter_messages
  <herokutl.client.messages.MessageMethods.iter_messages>`
  with `None` entity.
- New `client.inline_query <herokutl.client.bots.BotMethods.inline_query>`
  method to perform inline queries.
- Bot-API-style ``file_id`` can now be used to send files and download media.
  You can also access `herokutl.utils.resolve_bot_file_id` and
  `herokutl.utils.pack_bot_file_id` to resolve and create these
...
- New `herokutl.utils.get_peer`, useful when you expect a :tl:`Peer`.

Bug fixes
~~~~~~~~~

- UTC timezone for `herokutl.events.userupdate.UserUpdate`.
...
- `client.catch_up() <herokutl.client.updates.UpdateMethods.catch_up>`
...
The `custom.Message <herokutl.tl.custom.message.Message>` class has been
...
namely `events.InlineQuery <herokutl.events.inlinequery.InlineQuery>`
and `events.CallbackQuery <herokutl.events.callbackquery.CallbackQuery>`
...
- New `custom.Button <herokutl.tl.custom.button.Button>` class
...
- New events usable if you sign in as a bot: `events.InlineQuery
  <herokutl.events.inlinequery.InlineQuery>` and `events.CallbackQuery
  <herokutl.events.callbackquery.CallbackQuery>`.
...
.. code-block:: python
    ...
    from herokutl import TelegramClient, sync
    from herokutl.network import ConnectionHttp
...
- ``add_mark=`` is now back on ``utils.get_input_peer`` and also on
  `client.get_input_entity() <herokutl.client.users.UserMethods.get_input_entity>`.
- New `client.get_peer_id <herokutl.client.users.UserMethods.get_peer_id>`
...
<herokutl.client.messages.MessageMethods.iter_messages>` to support reversing
...
- Count was always `None` for `message.button_count
  <herokutl.tl.custom.message.Message.button_count>`.
...
- `client.start() <herokutl.client.auth.AuthMethods.start>` was completely
...
- Disconnecting could complain if the magic ``herokutl.sync`` was imported.
...
.. important::
    ...
    If you come from herokutl pre-1.0 you **really** want to read
...
There is a new magic ``herokutl.sync`` module to let you use **all** the
methods in the :ref:`TelegramClient <herokutl-client>` (and the types returned
...
- ``message.get_fwd_sender`` is now in `message.forward
  <herokutl.tl.custom.message.Message.forward>`.
...
- ``client.get_message_history`` is now `client.get_messages
  <herokutl.client.messages.MessageMethods.get_messages>`
- ``client.send_voice_note`` is now `client.send_file
  <herokutl.client.uploads.UploadMethods.send_file>` with ``is_voice=True``.
...
- `client.send_file <herokutl.client.uploads.UploadMethods.send_file>` now
  accepts external ``http://`` and ``https://`` URLs.
- You can use the :ref:`TelegramClient <herokutl-client>` inside of ``with``
  blocks, which will `client.start() <herokutl.client.auth.AuthMethods.start>`
  and `disconnect() <herokutl.client.telegrambaseclient.TelegramBaseClient.disconnect>`
  the client for you:
  ...
  <herokutl.client.auth.AuthMethods.start>` method since
...
- Passing a class to `message.get_entities_text
  <herokutl.tl.custom.message.Message.get_entities_text>` now works properly.
- Iterating messages from a specific user in private messages now works.

Enhancements
~~~~~~~~~~~~

- Both `client.start() <herokutl.client.auth.AuthMethods.start>` and
  `client.run_until_disconnected()
  <herokutl.client.updates.UpdateMethods.run_until_disconnected>` can
...
- If you relied on internals like the ``MtProtoSender`` and the
  ...
  <herokutl.network.mtprotosender.MTProtoSender>` and `TelegramBaseClient
  <herokutl.client.telegrambaseclient.TelegramBaseClient>` and they behave
  differently.
- Underscores have been renamed from filenames. This means
  ``herokutl.errors.rpc_error_list`` won't work, but you should
  have been using `herokutl.errors` all this time instead.
- `client.connect <herokutl.client.telegrambaseclient.TelegramBaseClient.connect>`
...
- Accessing `.sender <herokutl.tl.custom.message.Message.sender>`,
  `.chat <herokutl.tl.custom.message.Message.chat>` and similar may *not* work
  ...
  `.get_sender() <herokutl.tl.custom.message.Message.get_sender>`,
  `.get_chat() <herokutl.tl.custom.message.Message.get_chat>` instead while
...
- The above point means ``reply_message`` is now `.get_reply_message()
  <herokutl.tl.custom.message.Message.get_reply_message>`, and ``fwd_from_entity``
  is now `get_fwd_sender() <herokutl.tl.custom.message.Message.get_fwd_sender>`.
...
- Telegram's Terms Of Service are now accepted when creating a new account.
  ...
  were created before.
- The `method reference <https://tl.herokutl.dev/>`_ now shows
  which methods can be used if you sign in with a ``bot_token``.
- There's a new `client.disconnected
  <herokutl.client.telegrambaseclient.TelegramBaseClient.disconnected>` future
...
- You can filter `events.NewMessage <herokutl.events.newmessage.NewMessage>`
...
- Several fixes to `herokutl.events.newmessage.NewMessage`.
...
- `message.get_entities_text
  <herokutl.tl.custom.message.Message.get_entities_text>` properly
  supports filtering, even if there are no message entities.
- `message.click <herokutl.tl.custom.message.Message.click>` works better.
...
- Fixed a rare infinite loop bug in `client.iter_dialogs
  <herokutl.client.dialogs.DialogMethods.iter_dialogs>` for some people.
- Fixed ``TypeError`` when there is no `.sender
  <herokutl.tl.custom.message.Message.sender>`.
...
- ``__all__`` is now defined, so ``from herokutl import *`` imports sane
...
- The ``herokutl_generator/`` can now convert ``.tl`` files into ``.json``,
...
This update brings a new `herokutl.tl.custom.message.Message` object!

All the methods in the `herokutl.telegram_client.TelegramClient` that
...
`herokutl.tl.custom.message.Message` to go to its page.


Breaking Changes
~~~~~~~~~~~~~~~~

- The `herokutl.network.connection.common.Connection` class is now an ABC,
  and the old ``ConnectionMode`` is now gone. Use a specific connection (like
  `herokutl.network.connection.tcpabridged.ConnectionTcpAbridged`) instead.

Additions
~~~~~~~~~

- You can get messages by their ID with
  `herokutl.telegram_client.TelegramClient.get_messages`'s ``ids`` parameter:
...
- More convenience properties for `herokutl.tl.custom.dialog.Dialog`.
- New default `herokutl.telegram_client.TelegramClient.parse_mode`.
- You can edit the media of messages that already have some media.
- New dark theme in the online ``tl`` reference, check it out at
  https://tl.herokutl.dev/.
...
- `herokutl.telegram_client.TelegramClient.iter_messages`'s ``from_user``
  was failing if no filter had been set.
- `herokutl.telegram_client.TelegramClient.iter_messages`'s ``min_id/max_id``
  was being ignored by Telegram. This is now worked around.
- `herokutl.telegram_client.TelegramClient.catch_up` would fail with empty
  states.
- `herokutl.events.newmessage.NewMessage` supports ``incoming=False``
...
- Some clean-up in the ``herokutl_generator/`` package.
...
`herokutl.telegram_client.TelegramClient.catch_up` method. This feature needs
...
- Add ``search``, ``filter`` and ``from_user`` parameters to
  `herokutl.telegram_client.TelegramClient.iter_messages`.
- `herokutl.telegram_client.TelegramClient.download_file` now
...
- Faster `herokutl.telegram_client.TelegramClient.iter_messages` by
  sleeping only as much as needed.
- `herokutl.telegram_client.TelegramClient.edit_message` now supports
  omitting the entity if you pass a :tl:`Message`.
- `herokutl.events.raw.Raw` can now be filtered by type.
...
As a side note, the documentation at https://tl.herokutl.dev
...
- `herokutl.telegram_client.TelegramClient.forward_messages` now returns
  a single item instead of a list if the input was also a single item.

Additions
~~~~~~~~~

- New `herokutl.events.messageread.MessageRead` event, to find out when
...
- Some filter types (as a type) were not supported by
  `herokutl.telegram_client.TelegramClient.iter_participants`.
- `herokutl.telegram_client.TelegramClient.remove_event_handler` works.
- `herokutl.telegram_client.TelegramClient.start` works on all terminals.
- :tl:`InputPeerSelf` case was missing from
  `herokutl.telegram_client.TelegramClient.get_input_entity`.

Enhancements
~~~~~~~~~~~~

- The ``parse_mode`` for messages now accepts a callable.
- `herokutl.telegram_client.TelegramClient.download_media` accepts web previews.
- `herokutl.tl.custom.dialog.Dialog` instances can now be casted into
  :tl:`InputPeer`.
...
as ``from herokutl import TelegramClient, events``). This are pretty much
...
- New exact match feature on https://tl.herokutl.dev.
...
refer to `#425 <https://github.com/LonamiWebs/herokutl/issues/425>`_.
...
parsing <https://github.com/LonamiWebs/herokutl/commit/ee01724cdb7027c1e38625d31446ba1ea7bade92>`__
of certain TLObjects thanks to @stek29, and
`some <https://github.com/LonamiWebs/herokutl/commit/ed77ba6f8ff115ac624f02f691c9991e5b37be60>`__
`wrong
calls <https://github.com/LonamiWebs/herokutl/commit/16cf94c9add5e94d70c4eee2ac142d8e76af48b9>`__
...
The main feature of this release is that herokutl now has a custom
...
https://github.com/LonamiWebs/herokutl/commit/62ea77cbeac7c42bfac85aa8766a1b5b35e3a76c.
...
Modes <https://github.com/LonamiWebs/herokutl/wiki/Usage-Modes>`__ for
...
modes** <https://github.com/LonamiWebs/herokutl/blob/v0.13/herokutl/network/connection.py>`__.
...
fixed) and RPC errors are reported by default:
    ...
    herokutl will by default report all Remote Procedure Call errors to
...
herokutl and make it better with either suggestions, or pull request.
...
This update is overall an attempt to make herokutl a bit more user
...
`InputPeer <https://tl.herokutl.dev/types/input_peer.html>`__.
There also exist
`InputChannel <https://tl.herokutl.dev/types/input_channel.html>`__
and
`InputUser <https://tl.herokutl.dev/types/input_user.html>`__!
...
method <https://github.com/LonamiWebs/herokutl/commit/8fd0d7eadd944ff42e18aaf06228adc7aba794b5>`__
to pretty format them, if you ever need to inspect them.

The library now uses `the DEBUG
level <https://github.com/LonamiWebs/herokutl/commit/1f7ac7118750ed84e2165dce9c6aca2e6ea0c6a4>`__
everywhere, so no more warnings or information messages if you had
logging enabled.

The ``no_webpage`` parameter from ``.send_message`` `has been
renamed <https://github.com/LonamiWebs/herokutl/commit/0119a006585acd1a1a9a8901a21bb2f193142cfe>`__
...
-  Fixed some errors when installing herokutl via ``pip`` (for those
...
-  Markdown parsing is **not part** of herokutl itself anymore, although
...
.. warning::
    ...
    used herokutl ≤ v0.10. If you happen to upgrade to an higher version,
...
- New nice button to copy the ``from herokutl.tl.xxx.yyy import Yyy``
...
time Telegram decides to kick us off their servers, so now herokutl can
...
This brings herokutl a whole step closer to ``v1.0``, though more things
...
herokutl used to crash a lot when logging in for the very first time.
...
- The **documentation**, available online
  `here <https://tl.herokutl.dev/>`__, has a new search bar.
- Better **cross-thread safety** by using ``threading.Event``.
- More improvements for running herokutl during a **long period of time**.
...
-  **Logging support**, used by passing ``--herokutl-log=DEBUG`` (or
...
If you're one of those who runs herokutl for a long time (more than 30
...
You can now use two factor authentication with herokutl too! As internal
...
changes that make using herokutl a little bit easier. Now those who have
installed herokutl via ``pip`` can also take advantage of changes, such
...
herokutl is now available as a **`Python
package <https://pypi.python.org/pypi?name=herokutl>`__**! Those are
