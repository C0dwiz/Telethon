...
The :ref:`herokutl-client` doesn't offer a method for every single request
the Telegram API supports. However, it's very simple to *call* or *invoke*
any request defined in Telegram's API.

This section will teach you how to use what herokutl calls the `TL reference`_.
...
.. note::
    ...
    The reason to keep both https://tl.herokutl.dev and this
    documentation alive is that the former allows instant search results
    as you type, and a "Copy import" button. If you like namespaces, you
    can also do ``from herokutl.tl import types, functions``. Both work.

Telegram makes these ``.tl`` files public, which other implementations, such
as herokutl, can also use to generate code. These files are versioned under
...
"Types" as understood by TL are not actually generated in herokutl.
...
spelling out the package under ``herokutl.tl.functions.*``).
...
.. code-block:: python
    ...
    from herokutl.tl.functions import PingRequest
...
<herokutl.client.messages.MessageMethods.send_message>` didn't exist,
...
.. code-block:: python
    ...
    from herokutl.tl.types import InputPeerUser
...
user, use `client.get_entity() <herokutl.client.users.UserMethods.get_entity>`
instead:
...
every time its used, simply call `herokutl.utils.get_input_peer`:
...
.. note::
    ...
    <herokutl.client.users.UserMethods.get_input_entity>` for you when
    required, but it's good to remember what's happening.

After this small parenthesis about `client.get_entity
<herokutl.client.users.UserMethods.get_entity>` versus
`client.get_input_entity() <herokutl.client.users.UserMethods.get_input_entity>`,
...
`herokutl.errors.common.MultiError`. You can ``except`` this
and still access the successful results:

.. code-block:: python

    from herokutl.errors import MultiError
...
.. _TL reference: https://tl.herokutl.dev
.. _TL diff: https://diff.herokutl.dev
.. _PingRequest: https://tl.herokutl.dev/methods/ping.html
.. _use the search: https://tl.herokutl.dev/?q=message&redirect=no
