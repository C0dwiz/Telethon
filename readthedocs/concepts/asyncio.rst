...
Asynchronous IO makes a lot of sense in a library like herokutl.
...
.. code-block:: python
    ...
    # or
    from herokutl.sync import ...
    # or
    import herokutl.sync

The ``sync`` module rewrites most ``async def``
methods in herokutl to something similar to this:
...
Just create another task, as shown above. If you're using the herokutl
...
<herokutl.client.updates.UpdateMethods.run_until_disconnected>` does is
...
<herokutl.client.updates.UpdateMethods.run_until_disconnected>` either!
...
the loop, and herokutl will be happy with any approach.
...
<herokutl.client.auth.AuthMethods.start>`, `run_until_disconnected
<herokutl.client.updates.UpdateMethods.run_until_disconnected>`, and
`disconnect <herokutl.client.telegrambaseclient.TelegramBaseClient.disconnect>`
all support this.
