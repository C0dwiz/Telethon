...
Both the bot API and herokutl will add a minus sign (negate) the real chat ID
...
`message.chat_id` you will see `-123`. This ID helps herokutl know you're
...
Both the bot API and herokutl will "concatenate" ``-100`` to the real chat ID
so that you can tell at a glance, with just a number, the entity type.
...
will see `-1000000000456`. This ID helps herokutl know you're talking about a
:tl:`Channel`.
...
.. code-block:: python
    ...
    print(real_id)  # 456
    print(peer_type)  # <class 'herokutl.tl.types.PeerChannel'>
...
If there is no "mark" (no minus sign), herokutl will assume your identifier
...
.. _Chat type: https://tl.herokutl.dev/types/chat.html
