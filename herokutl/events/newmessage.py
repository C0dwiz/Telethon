...
class NewMessage(EventBuilder):
    ...
    class Event(EventCommon):
        ...
        to all effects as a `Message <herokutl.tl.custom.message.Message>`,
        so please **refer to its documentation** to know what you can do
        with this event.

        Members:
            message (`Message <herokutl.tl.custom.message.Message>`):
                This is the only difference with the received
                `Message <herokutl.tl.custom.message.Message>`, and will
                return the `herokutl.tl.custom.message.Message` itself,
                not the text.

                See `Message <herokutl.tl.custom.message.Message>` for
