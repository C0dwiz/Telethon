...
`herokutl.client.telegramclient.TelegramClient` which is subclass of them
all to provide the final unified interface while the methods can live in
different subclasses to be more maintainable.

The ABC is `herokutl.client.telegrambaseclient.TelegramBaseClient` and the
first implementor is `herokutl.client.users.UserMethods`, since calling
