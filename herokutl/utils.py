...
from .helpers import add_surrogate, del_surrogate, strip_text  # noqa: F401
...
# See https://github.com/LonamiWebs/herokutl/issues/1096.
...
def resolve_bot_file_id(file_id):
    """
    Given a Bot API-style `file_id <herokutl.tl.custom.file.File.id>`,
    returns the media it represents. If the `file_id <herokutl.tl.custom.file.File.id>`
    is not valid, `None` is returned instead.

    Note that the `file_id <herokutl.tl.custom.file.File.id>` does not have information
